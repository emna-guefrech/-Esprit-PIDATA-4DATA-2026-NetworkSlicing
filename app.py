...
DB_PASS = os.environ.get("DB_PASS", "")  # Mot de passe vide par défaut pour XAMPP
...
# ── MS-3 Configuration ───────────────────────────────────────────
MS3_URL = os.environ.get("MS3_URL", "http://localhost:5003")
...
def sync_to_ms3(slice_id: str, qos_score: float, congestion: str, data: dict, pipeline: str = "6G"):
    """Synchroniser la prédiction avec MS-3"""
    try:
        payload = {
            "slice_id": slice_id,
            "pipeline": pipeline,
        ...
        print(f"🔍 DEBUG MS-1 - MS3_URL: {MS3_URL}")
        print(f"🔍 DEBUG MS-1 - Envoi à MS-3: {payload}")
        response = requests.post(f"{MS3_URL}/dashboard/sync/prediction", 
                              json=payload, timeout=10)
        print(f"🔍 DEBUG MS-1 - Réponse MS-3 status: {response.status_code}")
        print(f"🔍 DEBUG MS-1 - Réponse MS-3 text: {response.text}")
        if response.status_code == 200:
            result = response.json()
            print(f"🔍 DEBUG MS-1 - MS-3 response: {result}")
            print(f"[MS-3] ✅ Slice {slice_id} synchronized")
        else:
            print(f"[MS-3] ❌ Error syncing slice {slice_id}: {response.status_code} - {response.text}")
    except requests.exceptions.ConnectionError as e:
            print(f"[MS-3] ❌ Connection Error: Impossible de se connecter à MS-3 à {MS3_URL}")
            print(f"[MS-3] ❌ Erreur: {e}")
    except Exception as e:
            print(f"[MS-3] ❌ Network error syncing to MS-3: {e}")
    else:
        print(f"[MS-3] ⚠️ Sync failed: {response.status_code}")
...
def index():
    ...
    normal = Prediction.query.filter(Prediction.qos_score >= 0.7).count()
    critical = Prediction.query.filter(Prediction.qos_score < 0.4).count()
    
    # Get recent predictions for display
    predictions = Prediction.query.order_by(Prediction.created_at.desc()).limit(10).all()
    
    return render_template('single_page.html', total=total, normal=normal, critical=critical, predictions=predictions)
...
def update_prediction(prediction_id):
    ...
    try:
        ...
        if 'slice_id' in data:
            prediction.slice_id = data['slice_id']
        if 'pipeline' in data:
            prediction.pipeline = data['pipeline']
        if 'congestion_level' in data:
            prediction.congestion_level = data['congestion_level']
        if 'qos_score' in data:
            prediction.qos_score = data['qos_score']
...
def predict_qos():
    ...
    try:
        ...
        # Sync to MS-3
        sync_to_ms3(slice_id, qos_score, congestion, data, "6G")
...
def predict_qos_5g():
    ...
    try:
        ...
        # Sync to MS-3
        sync_to_ms3(slice_id, qos_score, congestion, data, "5G")
        # Trigger alert if Critical
        if ALERTS_ENABLED and congestion == "Critical":
            send_alert(
...
def predict_congestion():
    ...
    try:
        ...
        # Sync to MS-3
        qos_score = round(1 - confidence if congestion == "Critical" else confidence, 4)
        sync_to_ms3(slice_id, qos_score, congestion, data, "6G")
        # Alert if Critical
        if ALERTS_ENABLED and congestion == "Critical":
            send_alert(
