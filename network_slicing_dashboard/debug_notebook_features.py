#!/usr/bin/env python3
"""
Debug des features exactes du notebook de Eya
"""
...
print("🔍 DEBUG DES FEATURES EXACTES DU NOTEBOOK")
...
print(f"Colonnes brutes: {list(df_reg.columns)}")
...
print(f"  Latency Budget: {latency_budget_col}")
print(f"  Slice Latency: {slice_latency_col}")
print(f"  Packet Loss Budget: {packet_loss_budget_col}")
print(f"  Slice Packet Loss: {slice_packet_loss_col}")
print(f"  Jitter Budget: {jitter_budget_col}")
print(f"  Slice Jitter: {slice_jitter_col}")
print(f"  Data Rate Budget: {rate_budget_col}")
print(f"  Slice Available Transfer Rate: {slice_rate_col}")

# Calculer les gaps exacts comme Eya
...
# Calculer les scores exacts comme Eya
...
print(f"\n🎯 QoS_Probability: min={df_reg['QoS_Probability'].min():.4f}, max={df_reg['QoS_Probability'].max():.4f}, mean={df_reg['QoS_Probability'].mean():.4f}")

# Features exactes utilisées pour l'entraînement
X_reg = df_reg[['Latency_Gap', 'Packet_Loss_Gap', 'Jitter_Gap', 'Rate_Gap']]
y_reg = df_reg['QoS_Probability']

print(f"\n🔧 Features d'entraînement:")
print(f"  Colonnes: {list(X_reg.columns)}")
print(f"  Shape: {X_reg.shape}")
print(f"  Target shape: {y_reg.shape}")

# Sauvegarder les features exactes
import os
os.makedirs('models', exist_ok=True)
joblib.dump(X_reg.columns.tolist(), 'models/features_regression_eya_debug.joblib')

print(f"\n✅ Features debug sauvegardées: models/features_regression_eya_debug.joblib")
